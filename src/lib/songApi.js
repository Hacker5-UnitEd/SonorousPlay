...
export async function init() {
  ...
  await node.bootstrap.add(
    "/dns4/wrtc-star2.sjc.dwebops.pub/tcp/443/wss/p2p-webrtc-star/p2p/QmfU4GLK2mcv53ebXuazeMKAJ55dfJ5g1ufm47NwDRR2vb"
  );
  return node;
}

export async function getUrl(node, path, useGatewayForLocal) {
  if (/* path.slice(0, 6) != "/ipfs/" */ true) {
...
export function addSongWithCompression(node, file, jsmediatags) {
  return new Promise((resolve, reject) => {
    jsmediatags.read(file, {
      onSuccess: async (tag) => {
        const albumName = tag.tags.album || "Unknown Album";
        ...
        await ffmpeg.run('-i', file.name, '-c:a', 'libfdk_aac', '-b:a', '320k', 'output.aac');
        console.log('Complete transcoding');
        const data = ffmpeg.FS('readFile', 'output.aac');

        await node.files.write(`/${artistName}/${albumName}/${songName}`, new Blob([data.buffer]), {
          create: true,
          parents: true,
        });
        if (tag.tags.picture) {
...
export async function getListFromHash(node, hash) {
  ...
  let artistList = {};
  let id = 0;
  for await (const artist of node.files.ls("/ipfs/"+hash)) {
    let song = { artist: artist.name };
    artistList[artist.name] = "";
    for await (const album of node.files.ls("/ipfs/"+hash + "/" + artist.name)) {
      if (album.name == "artistArt") {
        ...
        albumList[album.name] = "/ipfs/bafybeidn3kttyu6ethc7wae3xrvkfoupqsjzqns2q5mwxgsssrfcbcs5gq";
        for await (const aud of node.files.ls(
          "/ipfs/"+hash + "/" + artist.name + "/" + album.name
        )) {
          if (aud.name != "albumArt") {
            song.name = aud.name;
            ...
            albumList[album.name] =
              "/ipfs/"+hash + "/" + artist.name + "/" + album.name + "/" + aud.name;
          }
        }
      }
    }
...
export async function addRemoteSong(node, path, jsmediatags) {
  ...
  alert('The song is saved to your library!')
  return true;
}
